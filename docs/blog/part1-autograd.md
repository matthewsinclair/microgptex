...
This is an exercise in making it possible for any suitably motivated software engineer to understand the basic mechanics of a GPT. It is _not_ a complete, production-ready implementation — it's a learning tool. **Prerequisites:** comfort with basic calculus (derivatives, the chain rule), familiarity with what neural networks are (weights, activations, loss), and enough programming experience to read code in an unfamiliar language. No Elixir knowledge required — the patterns are explained as they appear. With that in mind, PRs are welcome. If something is wrong, or worse, subtly off-target, feel free to submit a fix. What I'd love to see over time is this getting refined and improved until it reflects both best practice and a genuinely useful learning resource.
...
These six primitives are sufficient to implement the entire GPT algorithm. Negation, subtraction, and division are derived from them (negation is `mul(a, -1)`, subtraction is `add(a, neg(b))`, division is `mul(a, pow(b, -1))`), so they inherit autograd support for free.
